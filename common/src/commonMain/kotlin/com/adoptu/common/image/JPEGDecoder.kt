...
// Ported to Kotlin by soywiz for the Korim/korge-image-formats project
// (https://github.com/korlibs/korge-image-formats/blob/main/korim-jpeg/src/commonMain/kotlin/korlibs/image/format/jpg/JPEGDecoder.kt)
// Vendored into this project rather than depending on korim, for the same reason as
// JPEGEncoder.kt - see that file's header. Adapted to drop korlibs.memory.UByteArrayInt and
// korlibs.io.lang.invalidOp (replaced with a local equivalent below) and
// korlibs.image.bitmap.Bitmap32 (decode() returns a plain width/height/RGBA-bytes result
// instead) - the decode algorithm itself is unchanged.
class JPEGDecoder {
	...
	companion object {
		...
		/** Returns interleaved RGBA bytes, width * height * 4 bytes total. */
		fun decode(jpegData: ByteArray): DecodedImage {
			val data = decodeToData(jpegData)
			return DecodedImage(data.width, data.height, data.data.asByteArray())
		}
...
/** Result of [JPEGDecoder.decode] - [rgba] is interleaved RGBA bytes, width * height * 4 bytes. */
class DecodedImage(val width: Int, val height: Int, val rgba: ByteArray)

private fun invalidOp(msg: String): Nothing = throw IllegalStateException(msg)

/** Minimal unsigned-byte array wrapper - stands in for korlibs.memory.UByteArrayInt. */
class UByteArrayInt {
