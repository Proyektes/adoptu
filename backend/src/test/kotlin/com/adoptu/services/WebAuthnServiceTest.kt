...
class WebAuthnServiceTest {
    ...
    @Test
    fun `registration challenge is persisted in the database, not process memory`() = runBlocking {
        val email = "persisted-challenge@example.com"
        webAuthnService.generateRegistrationOptions(email, "Persisted Challenge User")

        val stored = transaction {
            WebAuthnChallenges.selectAll()
                .where { WebAuthnChallenges.challengeKey eq "registration:$email" }
                .firstOrNull()
        }
        assertNotNull(stored)

        val challenge = Base64.getDecoder().decode(stored[WebAuthnChallenges.challenge])
        val keyMaterial = WebAuthnCeremony.generateKeyMaterial()
        val responseJson = WebAuthnCeremony.buildRegistrationResponseJson(rpId, origins.first(), challenge, keyMaterial)

        val result = webAuthnService.verifyAndRegister(email, "Persisted Challenge User", setOf(UserRole.ADOPTER), responseJson)
        assertNotNull(result)

        // Consuming the challenge removes its row - proves it isn't left to accumulate forever
        // like the old in-memory map (which only ever removed entries on retrieval).
        val afterConsume = transaction {
            WebAuthnChallenges.selectAll().where { WebAuthnChallenges.challengeKey eq "registration:$email" }.firstOrNull()
        }
        assertNull(afterConsume)
    }

    @Test
    fun `a challenge generated by one WebAuthnService instance can be completed by a different instance sharing the same database`() = runBlocking {
        // Simulates two separate ECS tasks: the load balancer can route the registration-options
        // request and the completion request to different instances/JVMs, each with its own
        // process memory - only a shared, DB-backed ChallengeStore makes this work.
        val otherInstanceWebAuthnService = WebAuthnService(
            clock, emailVerificationService, userService(), passwordService, magicLinkService, adminEmail, rpId, rpName, origins
        )

        val email = "cross-instance-passkey@example.com"
        val displayName = "Cross Instance User"
        val options = webAuthnService.generateRegistrationOptions(email, displayName)
        val challenge = Base64.getUrlDecoder().decode(options.publicKey.challenge)
        val keyMaterial = WebAuthnCeremony.generateKeyMaterial()
        val responseJson = WebAuthnCeremony.buildRegistrationResponseJson(rpId, origins.first(), challenge, keyMaterial)

        val result = otherInstanceWebAuthnService.verifyAndRegister(email, displayName, setOf(UserRole.ADOPTER), responseJson)
