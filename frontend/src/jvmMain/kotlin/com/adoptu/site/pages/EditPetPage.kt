...
// Dedicated add/edit page for a single pet - reached via "/edit-pet" (add) or "/edit-pet?id={id}"
// (edit) from Manage Pets' pet cards. Previously this form lived inline on /my-pets, toggled by a
// hidden class; that made "Edit" look broken (opened far above the scroll position with zero
// feedback) and crowded the page with every rescuer's other pets alongside the report sections.
fun HTML.editPetPage(navParams: NavParams = NavParams()) {
    commonHead("Edit Pet - Adopt-U", "mypets.css")
    body {
        header {
            a("/") { commonLogo() }
        ...
        main {
            ...
            form { id = "pet-form"
                ...
                div(classes = "form-actions") {
                    ...
                    a(href = "/my-pets", classes = "btn btn-secondary") { attributes["data-i18n"] = "cancel"; +"Cancel" }
                }
            }
